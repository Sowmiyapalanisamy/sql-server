...
-- Problem # 1:Write a query to display the member id, member name, city 
...
--Problem # 3:Write a query to display the member id, member name
...
--Problem # 4:Write a query to display the book code, book title
--and author of the books whose author name begins with 'P'
select book_code,book_title,author from 
lms_book_details where author like'p%';

--Problem # 5:Write a query to display the total number of Java books
--available in library with alias name ‘NO_OF_BOOKS’
select count(category) as no_of_books from lms_book_details; 

--Problem # 6:Write a query to list the category and number 
--of books in each category with alias name ‘NO_OF_BOOKS’.
select  category,count(category) as no_of_books from 
lms_book_details group by category; 

--Problem # 7:Write a query to display the number of books 
--published by "Prentice Hall” with the alias name “NO_OF_BOOKS”.
select count(book_title)  as no_of_books from lms_book_details
...
--Problem # 9:Write a query to display the member id, member name, 
--date of registration and expiry date of the members whose membership 
--expiry date is before APR 2013.

select member_id,member_name,date_register,date_expire from lms_members
where date_expire< '2013-04-01'; 

--Problem # 10:write a query to display the member id, member name,
--date of registration, membership status of the members who registered 
--before  "March 2012" and membership status is "Temporary" 
...
--who’s City is CHENNAI or DELHI. Hint: Display the member name in 
--title case with alias name 'Name'. 

select member_id,member_name from lms_members
where city='chennai' or city='delhi';

--problem # 13 :Write a query to display the average price of books
--which is belonging to ‘JAVA’ category with alias name “AVERAGEPRICE”.

select avg(price) as averageprice
from lms_book_details where category='java';

--Problem #14:Write a query to display the supplier id,
--supplier name and email of the suppliers who are all 
--having gmail account.
select * from lms_suppliers_details;
select supplier_id,supplier_name,email 
from lms_suppliers_details where  email like '%gmail.com';
