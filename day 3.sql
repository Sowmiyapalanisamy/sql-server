Create Database day3Employee_DB;
Use day3Employee_DB;
...
--subquery is select query inside another query
--two types 1.noncorelated 2.corelated

select first_name,email,salary from employee where department_id=
(select id from department where name='hr') 

--display state of india
select state from location where country_id=(select id from countries 
where name='india') and city='bombay'

--2.	Write a query to display the names of all states belonging 
--to the country Canada. Display the records sorted in ascending order based on state name
...
--Display the records sorted in ascending order based on manager name
 select first_name  from employee where department_id in(select id from department where name='hr') order by first_name;
 ...
 select first_name from employee where id in(select id from manager) order by first_name;
...
select name from countries where region_id in( select id from region
where name='europe')order by name;
...
--group by
select department_id,count(id) as employeeineachdept from employee group by department_id; 
 
select country_id,count(state) from location
group by country_id;

-- 7 .	Write a query to display the department name and the number of managers
--in the department. Display the records sorted in ascending order based on 
--department name. Give an alias to the number of managers as manager_count
 select name ,count(name)from department group by name order by name;